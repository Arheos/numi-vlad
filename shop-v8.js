(function () {
  ...
  const items = [
    { id: 'g2-pro', name: 'KUKIRI G2 PRO', price: 100, image: 'shop-g2-v7.webp', label: 'ELECTRICĂ', badge: 'G2 PRO' },
...
.shop-wallet{position:fixed;top:calc(max(14px,env(safe-area-inset-top)) + 12px);left:max(18px,env(safe-area-inset-left));z-index:30;display:flex;align-items:center;gap:10px;height:42px;padding:0 14px;border-radius:16px;background:#fff9ed;border:1px solid #f4e3bd;box-shadow:0 4px 15px #00000008;color:#24201a;font-weight:800;font-size:16px}
.shop-wallet svg{width:23px;height:23px;color:#d89410}.shop-wallet small{font-size:11px;font-weight:700;color:#786c57}
...
#scooterShop:not([open]){display:none}#scooterShop::backdrop{background:#050505b8;backdrop-filter:blur(9px)}#scooterShop[open]{animation:shopEnter .26s ease-out}
@keyframes shopEnter{from{opacity:0;transform:translateY(18px) scale(.98)}to{opacity:1;transform:none}}
.shop-shell{padding:22px 22px 18px;max-width:500px;margin:auto;position:relative}.shop-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:16px}
.shop-eyebrow{margin:0 0 7px;font-size:10px;font-weight:800;letter-spacing:2px;color:#fd9b42;display:flex;align-items:center;gap:6px}.shop-eyebrow:before{content:'';width:6px;height:6px;border-radius:50%;background:#fd9b42}
.shop-title{font-size:42px;line-height:1;font-weight:900;letter-spacing:-1.8px;margin:0;color:#fff}.shop-subtitle{font-size:12px;line-height:1.5;color:#aba9a7;margin:8px 0 0}
.shop-close{flex:0 0 auto;width:42px;height:42px;display:grid;place-items:center;border-radius:50%;border:1px solid #ffffff23;background:#202225;color:#fff;font-size:27px;font-weight:400;line-height:1;transition:background .15s}.shop-close:hover{background:#33363a}
.shop-balance{display:flex;align-items:center;justify-content:space-between;padding:12px 15px;margin-bottom:18px;border:1px solid #7c532b;background:linear-gradient(115deg,#302317,#1d1d1e);border-radius:17px;gap:12px}.shop-balance-label{color:#d5c9ba;font-size:11px;letter-spacing:1px;font-weight:700}.shop-balance-value{display:flex;align-items:center;gap:8px;font-size:23px;font-weight:900;color:#ffc66b;font-variant-numeric:tabular-nums}.shop-balance svg{width:23px;height:23px}.shop-balance-value small{font-size:12px;font-weight:700}
...
.shop-reward-rule{padding:10px 12px;border:1px solid #f6ac4933;border-radius:12px;background:#f6ac490c;color:#ffd28d;font-size:12px;line-height:1.5;margin:0 0 16px}
`;
...
<dialog id="scooterShop" aria-labelledby="shopTitle" aria-describedby="shopSubtitle">
 <div class="shop-shell">
  <header class="shop-head"><div><p class="shop-eyebrow">GARAJUL TĂU</p><h2 class="shop-title" id="shopTitle">SHOP</h2><p class="shop-subtitle" id="shopSubtitle">Două trotinete. Alege-ți favorita.</p></div><button id="closeShop" class="shop-close" aria-label="Închide magazinul" autofocus>×</button></header>
  ...
  <footer class="shop-footer"><span class="shop-collection">COLECȚIE · <span id="ownedCount">0</span> / 2</span><button id="backToGame" class="shop-back">← Înapoi la joc</button></footer>
  <p class="shop-save-note">Trofeele și colecția se păstrează și când începi o rundă nouă.</p>
 </div>
</dialog>`;
  ...
  function readState() {
    try {
      ...
      if (!saved || typeof saved !== 'object' || Array.isArray(saved)) return { balance: 0, owned: [], levelClaims: [] };
      ...
      return { ...saved, balance: Number.isSafeInteger(balance) && balance >= 0 ? balance : 0,
        ...
        levelClaims: Array.isArray(saved.levelClaims) ? Array.from(new Set(saved.levelClaims.filter(value => typeof value === 'string' && /^r[a-z0-9-]{5,80}:[12]$/.test(value)))) : [] };
  ...
  function drawShop() {
    ...
    items.forEach(item => {
      ...
      button.disabled = owned || missing > 0;
      button.classList.toggle('owned', owned);
      button.textContent = owned ? '✓ Deținută' : 'Cumpără';
      button.setAttribute('aria-label', owned ? item.name + ' este în colecție' : 'Cumpără ' + item.name + ' pentru ' + item.price + ' trofee');
      id('note-' + item.id).textContent = owned ? 'În colecția ta.' : missing ? 'Îți mai lipsesc ' + missing + (missing === 1 ? ' trofeu.' : ' trofee.') : 'Pregătită pentru colecția ta.';
    });
    if (!storageOK) id('shopMessage').textContent = 'Salvarea nu este disponibilă în acest browser.';
  }
  function grantLevel(runId, level) {
    if (typeof runId !== 'string' || !/^r[a-z0-9-]{5,80}$/.test(runId) || (level !== 1 && level !== 2)) return -1;
    ...
    // The balance and receipt are one atomic localStorage record.
  ...
  dialog.addEventListener('click', event => {
    ...
    drawShop(); id('shopMessage').textContent = '✓ ' + item.name + ' a intrat în colecția ta!';
    id('closeShop').focus({ preventScroll: true });
  });
  items.forEach(item => id('scooter-' + item.id).addEventListener('error', () => {
    id('shopMessage').textContent = 'O poză nu s-a încărcat. Reîncarcă pagina pentru a o vedea.';
  }));
