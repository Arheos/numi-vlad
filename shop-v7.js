...
<dialog id="scooterShop" aria-labelledby="shopTitle" aria-describedby="shopSubtitle">
 <div class="shop-shell">
  ...
  <p class="shop-save-note">Trofeele și colecția se salvează pe acest dispozitiv.</p>
 </div>
</dialog>`;
  ...
  let memory = { balance: 0, owned: [] }, storageOK = true;
  function readState() {
    try {
      const raw = localStorage.getItem(KEY);
      if (!raw) return { balance: 0, owned: [] };
      const saved = JSON.parse(raw);
      if (!saved || typeof saved !== 'object' || Array.isArray(saved)) return { balance: 0, owned: [] };
      const balance = Number(saved.balance);
      return { balance: Number.isSafeInteger(balance) && balance >= 0 ? balance : 0,
  ...
  dialog.addEventListener('click', event => {
    ...
    if (!item || state.owned.includes(item.id) || state.balance < item.price) { drawShop(); return; }
    const updated = { balance: state.balance - item.price, owned: state.owned.concat(item.id) };
  ...
  // Trophy earning has not been specified. Start at 0; do not alter scores or award currency.
  drawShop();
})();
